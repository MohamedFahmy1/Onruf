...
const Products = () => {
  ...
  const getProductsAndFolders =async() => {
    const [
      ...
      await axios.get(
        `${process.env.REACT_APP_API_URL}/ListProductByBusinessAccountId?currentPage=1&lang=${locale}`),
      await axios.get(`${process.env.REACT_APP_API_URL}/ListFolder?type=1&pageIndex=1&PageRowsCount=10&lang=${locale}`),
    ])
  ...
  const addNewFolder = async () => {
    try {
      if (!createNewFolder) {
        ...
        const res = await axios.post(process.env.REACT_APP_API_URL + '/AddFolder', formData)
        await axios.post(process.env.REACT_APP_API_URL + '/AddFolderProduct', {
  ...
  const addProductToFolder = async (id) => {
    if (!productsIds?.length) return toast.warning(locale === 'en' ? 'No products were selected!' : "من فضلك قم بأضافة المنتجات")
  ...
  const handleRemoveProduct = async () => {
    if (!productsIds?.length) return toast.warning(locale === 'en' ? 'No products were selected!' : "من فضلك قم بأضافة المنتجات")
    try {
      const isDelete = confirm(locale === 'en' ? 'Are you sure you want to delete this product ?' : 'هل ترغب في مسح تلك المنتجات ؟')
      if(!isDelete) return
      await axios.delete(process.env.REACT_APP_API_URL + `/RemoveListProductByBusinessAccount`, { data: productsIds })
      setOpenFolderModal(false)
      toast.success(locale === 'en' ? 'Products has been deleted successfully!' : 'تم حذف المنتج بنجاح')
      const { data: { data } } = await axios(`${process.env.REACT_APP_API_URL}/ListProductByBusinessAccountId?currentPage=1&lang=en`)
      setProducts(data)
    }
    catch (error) {
      console.error(error)
  ...
  return (
    <div>
      <ViewProducts products={products} setProductsIds={setProductsIds} selectedRows={selectedRows} setSelectedRows={setSelectedRows}/>
      {/* Folder Modal */}

      <Modal show={openFolderModal} onHide={() => setOpenFolderModal(false)}>
        <Modal.Header>
          <h5 className="modal-title m-0 f-b" id="staticBackdropLabel">
            {!createNewFolder ? pathOr('', [locale, 'Products', 'addNewFolder'], t) : pathOr('', [locale, 'Products', 'selectFolder'], t)}
          </h5>
        ...
        <Modal.Body>
          {!createNewFolder ? (
            ...
            <ul className="list_folder scroll-modal-folders">
              {folders?.fileList?.filter(({ isActive }) => (isActive)).map(({ id, image, name, fileProducts }) => <li className="item" key={id}>
                <div>
                  <img src={image} />
                  <div>
                    <h6 className="f-b">{name}</h6>
                    <div className="gray-color">
                      <span className="main-color f-b">{fileProducts?.length}</span> {locale === 'en' ? 'added products' : 'منتج مضاف'}
                    </div>
                  </div>
                </div>
                <button className="btn-main" onClick={() => addProductToFolder(id)}>
                  ...
                  locale === 'en' ? 'Save' : pathOr('', [locale, 'Products', 'save'], t)
                  }
                </button>
              </li>)}
            </ul>
        ...
        <Modal.Footer className="modal-footer">
          <button type="button" className="btn-main" onClick={addNewFolder}>
            {!createNewFolder ? pathOr('', [locale, 'Products', 'save'], t) : pathOr('', [locale, 'Products', 'addNewFolder'], t)}
          </button>
        </Modal.Footer>
      </Modal>
      ...
      <div className="btns_fixeds">
        <button className="btn-main rounded-0" onClick={handleRemoveProduct}>
          {locale === 'en' ? 'Delete selected' : 'حذف المحدد'}<RiDeleteBin5Line />
        </button>
        <button onClick={() => {
          if (!productsIds?.length) return toast.warning(locale === 'en' ? 'No products were selected!' : "من فضلك قم بأضافة المنتجات")
