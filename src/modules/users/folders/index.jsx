...
const UsersFolders = () => {
  ...
  return (
    <>
        ...
        <div>
          <div className="d-flex align-items-center justify-content-between mb-4 gap-2 flex-wrap">
            <div className="d-flex align-items-center">
              <h6 className="f-b m-0">مجلدات العملاء ({ totalNumberOfUsers })</h6>
              <Link href="/users">
                <a className="btn-main btn-main-w mr-20">
                  رجوع الي صفحة جميع العملاء <img src={rightArrow.src} />
                </a>
              </Link>
            </div>
            <button className="btn-main" onClick={() => {setOpenFolderModal(!openFolderModal);setEditModal(false)}}>
            ...
            <div>
              <Row>
                  {folders&&folders?.map((folder,index) => (
                    <Col lg={4} md={6} key={folder?.id + index }>
                      <div className="box_cus_Folder"  onClick={() => {setEditedFolderName(folder.name);setEditModal(true);setFolderId(folder.id)}} >
                    ...
                    <div onClick={() => Router.push(`/users/folders/${folder?.id}`)}>
                        ...
                        <div className="gray-color">
                          <span className="main-color f-b">{folder?.fileUser?.length}</span> عميل مضاف
                        </div>
                        <div className="avatars-stack">
                          {folder.fileUser?.slice(0, 5).map((user) => (
              ...
              {totalNumberOfUsers > pageSize  && <Pagination listLength={totalNumberOfUsers} pageSize={pageSize} />}
            </div>
          </div>
        </div>
      </div>
      ...
      <Modal show={openFolderModal} onHide={() =>{ setOpenFolderModal(false);setEditedFolderName("");setFolderName("")}}>
        <Modal.Header>
          <h5 className="modal-title m-0 f-b" id="staticBackdropLabel">
            {!folders?.length ? "اضافة مجلد  جديد" : "اختر المجلد"}
          </h5>
