...
const Wallet = () => {
  ...
  const handleWalletSubmit = async (values) => {
    ...
    try {
      const result = await axios.post(process.env.REACT_APP_API_URL + "/AddWalletTransaction",
      formData
      //  {
          ...
          <Row className="justify-content-between">
            <Col lg={5}>
              <div className="info_sec_ mb-3">
                <div className="icon">
                  <img src={"../core/imgs/wallet_icon.svg"} className="img-fluid" />
                </div>
                <h4 className="gray-color m-0">{pathOr("", [locale, "Wallet", "myBalance"], t)}</h4>
                <h5 className="f-b m-0">{walletBalance} SAR</h5>
              </div>
            </Col>
