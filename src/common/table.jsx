...
const Table = ({ columns, data = [], pageSize = 10, isCheckbox = true, selectedRows = {}, onSelectedRowsChange = () => null }) => {
  const id = useId()
  ...
  return (
    <table className="table table_dash" {...getTableProps()}>
      ...
      <tbody {...getTableBodyProps()}>
        {rows?.length ? rows?.slice(((page - 1) * pageSize), (page) * pageSize).map((row, i) => {
          prepareRow(row)
          return (
    ...
    <tr>
      <td className='text-center' rowSpan={3} colSpan={8}><b>No data found</b></td>
    </tr>
      }
      </tbody>
    </table>
